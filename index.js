...
function fenomenSil(feno,index) {
  /*kod*/
  const yeniFenola = [...feno];
  yeniFenola.splice(index,1);
  return yeniFenola;
}
...
1. ilk parametre olarak fenomenler dizisini alın
2. İKİNCİ, ÜÇÜNCÜ, DÖRDÜNCÜ, BEŞİNCİ ve ALTINCI parametre olarak sırasıyla bir sırano (number), bir profil ismi (profile), takipçi sayısı (followers), 
gönderim sayısı  (posts) ve bir platform adı (platform) alın.
...
ÖRNEK: fenomenEkle(fenomenler, 6, "Workintech", 10000000, 2022, "Instagram") çağrıldığında dizinin sonuna 
yukarıdaki nesne en sona eklenerek yeni fenomenler dizisini döndürmelidir. */

function fenomenEkle(feno,sirano,isim,takipciSay,gonderimSay,platformAd) {
  /*kod*/
  const ekleFeno = [...feno];
  const yeniEkle =  {
    "number": sirano,
    "profile": isim,
    "followers": takipciSay,
    "posts": gonderimSay,
    "platform": platformAd
  };
...
function platformaGoreCokGonderiYapanFenomen(feno,platformAdi) {
  /*kod*/
  let enFazla = 0;
  let yeri = 0;
  for (let i in feno ){
  ...
  console.log(feno[yeri].profile);
  return feno[yeri].profile;
}
